...
class NoiseInjector(Node):
    def __init__(self):
        super().__init__('noise_injector', allow_undeclared_parameters=True, automatically_declare_parameters_from_overrides=True)
        ...
        self.MOTOR_LINEAR_NOISE = cast(float, self.get_parameter_or("motor_linear_noise", Parameter("motor_linear_noise", Parameter.Type.DOUBLE, 0.1)).value) # m/s
        self.MOTOR_ANGULAR_NOISE = cast(float, self.get_parameter_or("motor_angular_noise", Parameter("motor_angular_noise", Parameter.Type.DOUBLE, 0.05)).value) # rad/s
        self.MOTOR_STEP_INTERVAL = cast(float, self.get_parameter_or("motor_step_interval", Parameter("motor_step_interval", Parameter.Type.DOUBLE, 0.2)).value) # seconds

        # Encoder interval should match motor step interval
        self.ENCODER_LINEAR_NOISE = cast(float, self.get_parameter_or("encoder_linear_noise", Parameter("encoder_linear_noise", Parameter.Type.DOUBLE, 0.05)).value) # m/s
        self.ENCODER_LINEAR_NOISE_RATIO = cast(float, self.get_parameter_or("encoder_linear_noise_ratio", Parameter("encoder_linear_noise_ratio", Parameter.Type.DOUBLE, 0.01)).value) # m/s
        self.ENCODER_ANGULAR_NOISE = cast(float, self.get_parameter_or("encoder_angular_noise", Parameter("encoder_angular_noise", Parameter.Type.DOUBLE, 0.1)).value) # rad/s
        self.ENCODER_ANGULAR_NOISE_RATIO = cast(float, self.get_parameter_or("encoder_angular_noise_ratio", Parameter("encoder_angular_noise_ratio", Parameter.Type.DOUBLE, 0.1)).value) # rad/s
        self.ENCODER_INTERVAL = cast(float, self.get_parameter_or("encoder_interval", Parameter("encoder_interval", Parameter.Type.DOUBLE, 0.2)).value) # seconds

        self.GPS_X_NOISE = cast(float, self.get_parameter_or("gps_x_noise", Parameter("gps_x_noise", Parameter.Type.DOUBLE, 0.1)).value) # m
        self.GPS_Y_NOISE = cast(float, self.get_parameter_or("gps_y_noise", Parameter("gps_y_noise", Parameter.Type.DOUBLE, 0.1)).value) # m
        self.GPS_INTERVAL = cast(float, self.get_parameter_or("gps_interval", Parameter("gps_interval", Parameter.Type.DOUBLE, 1.0)).value) # seconds
