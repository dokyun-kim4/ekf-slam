...
class PathVizNode(Node):
    def __init__(self):
        ...
        self.ekf_beacon_viz_pub = self.create_publisher(
            MarkerArray, "/ekf_beacon_viz", 10
        )
        ...
        self.create_subscription(
            PoseWithCovarianceStamped, "/ekf_prediction", self.plot_ekf, 10
        )
        self.create_subscription(
            BeaconData, "/beacon_ground_truth", self.plot_beacons, 10
        )
        self.create_subscription(
            PoseWithCovarianceStamped,
            "/ekf_beacon_prediction",
            self.plot_ekf_beacons,
            10,
        )
    ...
    def plot_dead_reckoning(self, msg: Twist):
        ...
        if self.last_twist_time is None:
            ...
            if self.latest_gt_pose is not None:
                ...
                quat = [
                    self.latest_gt_pose.pose.pose.orientation.x,
            ...
            self.get_logger().info(
                f"Initializing dead reckoning at ({x:.3f}, {y:.3f}, {theta:.3f})"
            )
            return
    ...
    def plot_ekf(self, msg: PoseWithCovarianceStamped):
        """
        Plot the EKF SLAM estimated path of the robot as a Path message in RViz2.
        The PoseWithCovarianceStamped message from the `ekf_prediction` topic is converted to a PoseStamped message and appended to the Path message.

        Args:
            msg (PoseWithCovarianceStamped): PoseWithCovarianceStamped message containing the EKF SLAM estimated pose of the robot.
        """
        pose_msg = PoseStamped()
        pose_msg.header.stamp = self.get_clock().now().to_msg()
        pose_msg.header.frame_id = "odom"
    ...
    def plot_beacons(self, msg: BeaconData):
        """
        Plot the ground truth positions of beacons as cylinder markers in RVi2. 
        
        Args:
            msg (BeaconData): BeaconData message containing the ground truth positions of the beacons.
        """
    ...
    def plot_ekf_beacons(self, msg: PoseWithCovarianceStamped):
        ...
        Args:
            msg (PoseWithCovarianceStamped): PoseWithCovarianceStamped message containing the EKF SLAM estimated pose of a beacon.
        """
