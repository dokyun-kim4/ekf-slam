...
from .utils.utils import wrap_angle, Beacon, BEACON_SDF
...
class BeaconNode(Node):
    ...
    def __init__(self):
        ...
        self.beacon_ids = cast(
            List[int],
            self.get_parameter_or(
                "beacon_ids",
                Parameter("beacon_ids", Parameter.Type.INTEGER_ARRAY, [1, 2, 3]),
            ).value,
        )
        self.beacon_x_pos = cast(
            List[float],
            self.get_parameter_or(
                "beacon_x_pos",
                Parameter(
                    "beacon_x_pos", Parameter.Type.DOUBLE_ARRAY, [1.0, -1.0, -2.0]
                ),
            ).value,
        )
        self.beacon_y_pos = cast(
            List[float],
            self.get_parameter_or(
                "beacon_y_pos",
                Parameter("beacon_y_pos", Parameter.Type.DOUBLE_ARRAY, [1.0, 1.5, 2.0]),
            ).value,
        )
        self.RANGE_NOISE = cast(
            float,
            self.get_parameter_or(
                "range_noise", Parameter("range_noise", Parameter.Type.DOUBLE, 0.1)
            ).value,
        )
        self.RANGE_PROP_NOISE = cast(
            float,
            self.get_parameter_or(
                "range_prop_noise",
                Parameter("range_prop_noise", Parameter.Type.DOUBLE, 0.05),
            ).value,
        )
        self.BEARING_NOISE = cast(
            float,
            self.get_parameter_or(
                "bearing_noise", Parameter("bearing_noise", Parameter.Type.DOUBLE, 0.08)
            ).value,
        )
        self.MAX_RANGE = cast(
            float,
            self.get_parameter_or(
                "max_range", Parameter("max_range", Parameter.Type.DOUBLE, 2.5)
            ).value,
        )
        self.INTERVAL = cast(
            float,
            self.get_parameter_or(
                "interval", Parameter("interval", Parameter.Type.DOUBLE, 2.0)
            ).value,
        )
        ...
        self.beacon_gt_pub = self.create_publisher(
            BeaconData, "/beacon_ground_truth", 10
        )
    ...
    def spawn_beacons(self):
        ...
        if not self.spawn_client.wait_for_service(timeout_sec=2.0):
            self.get_logger().warn(
                "Gazebo /spawn_entity service not available. Beacons will not be visually spawned."
            )
            return
        ...
        for i, beacon in enumerate(self.beacons):
            ...
            self.get_logger().info(
                f"Requested visual spawn for {name} at ({beacon.x}, {beacon.y})"
            )
    ...
    def beacon_callback(self):
        ...
        for beacon in self.beacons:
            ...
            gt_msg.bearings.append(wrap_angle(bearing))
            ...
            noisy_msg.ranges.append(
                random.gauss(range, self.RANGE_NOISE + self.RANGE_PROP_NOISE * range)
            )
            noisy_msg.bearings.append(
                wrap_angle(random.gauss(bearing, self.BEARING_NOISE))
            )
